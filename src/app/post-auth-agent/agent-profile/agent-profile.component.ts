...
import { updateAgentProfileInfo } from 'src/app/state/auth/auth.actions';
import { deleteProfilePic, updateProfilePic } from 'src/app/state/auth/auth.actions';
...
export class AgentProfileComponent implements OnInit {
  ...
  constructor(
    ...
    private aggentService: AgentService,
    public zone: NgZone,
 ...
 async  ngOnInit() {
    ...
    this.getAuthState.subscribe((authData) => {
      ...
      if(this.authData.isAuthenticated) {
        ...
        this.loginUserData = this.authData.authMeta.agent;
  ...
  updateUserProfileDetails(payload) {
    ...
    console.log('this.loginUserData.userId', this.loginUserData);
    this.aggentService.updateAgentProfile(this.loginUserData.id, payload).subscribe({
      next: data => {
  ...
  async uploadUserProfileImage(capturedImage) {
    try {
      ...
      await this.loaderService.createLoading('Please wait while we change your profile image..')
    	...
    	const uploadLocations = this.authService.addUserProfileImage(`customers` , this.authData.authMeta.agent.id , uploadLocationPayload);
  ...
  saveInfoToBE(locationData) {
    console.log('Need to pass the id to update here , WIP');
    ...
    this.authService.saveProfileImage(this.employee?.id, payload).subscribe({
      next: (resP) => {
        this.store.dispatch(
          updateProfilePic({
